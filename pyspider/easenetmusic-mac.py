...
# edge_driver = r'/usr/local/bin/msedgedriver'
name = input('请输入带下载歌名：')
# 初始化browser对象
# browser = Edge(options=options, executable_path=edge_driver, capabilities={})
# 很奇怪Edge()函数只能接收下列两个参数，不然就报错。网上也有很多相关的内容。
# browser = Edge(r'/usr/local/bin/msedgedriver', {})
browser = Edge(executable_path=r'/usr/local/bin/msedgedriver', capabilities={}) 

# 获取音乐名称 id 演唱者
def get_name_id_singer(url):
    browser.get(url=url)
    browser.switch_to.frame('g_iframe') # 搜索结果都放在一个名为‘g_iframe’的框架里
    sleep(1)
    # 在当前框架里执行js代码，该代码目的是获取整个HTML源代码
